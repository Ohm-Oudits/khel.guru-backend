...
const liveRoutes = getRouteEntries(liveRouter);
const slotsRoutes = getRouteEntries(slotsRouter);
...
expectRoute(liveRoutes, "/", ["GET"]);
expectRoute(liveRoutes, "/:slug", ["GET"]);
expectRoute(liveRoutes, "/:slug/launch", ["POST"]);
expectRoute(liveRoutes, "/:slug/play", ["POST"]);

expectRoute(slotsRoutes, "/", ["GET"]);
expectRoute(slotsRoutes, "/:slug", ["GET"]);
expectRoute(slotsRoutes, "/:slug/launch", ["POST"]);
expectRoute(slotsRoutes, "/:slug/spin", ["POST"]);
expectRoute(slotsRoutes, "/wallet/balance", ["POST"]);
expectRoute(slotsRoutes, "/wallet/bet", ["POST"]);
expectRoute(slotsRoutes, "/wallet/win", ["POST"]);
expectRoute(slotsRoutes, "/wallet/rollback", ["POST"]);

expectRoute(sportRoutes, "/catalog", ["GET"]);
expectRoute(sportRoutes, "/participants/:participantId/logo", ["GET"]);
...
assert.equal(liveRoutes.length, 4, "Live router surface changed unexpectedly");
assert.equal(slotsRoutes.length, 8, "Slots router surface changed unexpectedly");
assert.equal(sportRoutes.length, 9, "Sports router surface changed unexpectedly");
