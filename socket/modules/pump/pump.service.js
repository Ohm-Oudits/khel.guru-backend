...
// One active bet per user for the current pump round. The entry is created
// when the stake is debited and removed exactly once at settlement (cashout
// or pop), which is what guarantees a single debit and a single credit.
...
const service = {
  ...
  // A popped balloon discards the bet: the stake debit stands, nothing is
  ...
  // as a pop: the stake debit stands).
  clearBet(userId) {
    activeBets.delete(userId);
  },
};
